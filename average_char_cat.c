...
int wordCount() {
  ...
  { //while loop that gets characters until EOF
    charnum = charnum + 1; //increment character counter every getchar()
    
    if ((ispunct(ch)==0) && !(isspace(ch)))
    { //if ch is not space or punctuation increment the word-character
      wordcharnum = wordcharnum + 1; 
    }
    ...
    if (ch == ' ' || ch == '\n') 
    {
      ...
      dchar = prevchar; 
      //create variable that will be two characters back from next get char to check for double spaces
    }
    ...
    else if ((isprevspace && dchar != ' ' && ispunct(ch) == 0) || (dchar == ' ' && ispunct(ch)== 0 && (ispunct(prevchar) || prevchar == ' '))) 
    {
     //basically if this is a new word (preceeded by whitespace) even if double spaced between word or a punctuation before
      wordnum = wordnum + 1;
      isprevspace = 0; } //change the previous char being space boolean to false
  ...
  if (wordnum != 0)
  { //if number of words is zero dont do division by zero
    avgchar = (float)wordcharnum/wordnum;
  }
printf("Avg word length: %.1f characters\n", avgchar);
  return 0;
}

int main(){ //main method to run function that does the word counting
  return wordCount();
